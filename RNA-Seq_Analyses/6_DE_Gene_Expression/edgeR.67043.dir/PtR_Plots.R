...
if (file.exists("PtR_Plots.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("PtR_Plots.RData")
} else {
...
write.table(data, file="PtR_Plots.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
write.table(sample_cor, file="PtR_Plots.log2.centered.sample_cor.dat", quote=F, sep='	')
sample_dist = dist(t(data), method='euclidean')
hc_samples = hclust(sample_dist, method='complete')
pdf("PtR_Plots.log2.centered.sample_cor_matrix.pdf")
...
", "PtR_Plots.log2.centered") , ColSideColors=sampleAnnotations, RowSideColors=t(sampleAnnotations))
dev.off()
pdf("PtR_Plots.log2.centered.prcomp.principal_components.pdf")
prin_comp_data = data
...
write.table(pca$rotation, file="PtR_Plots.log2.centered.PCA.prcomp.scores", quote=F, sep="	")
write.table(pca$x, file="PtR_Plots.log2.centered.PCA.prcomp.loadings", quote=F, sep="	")
PCA.loadings=pca$x
PCA.scores = pca$rotation
for (i in 1:(max(4,2)-1)) {
...
par(def.par)
pcloadings_mat_vals = PCA.loadings[,1:4]
