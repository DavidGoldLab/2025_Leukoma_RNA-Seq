...
factor_labeling = read.table("salmon.gene.counts.matrix.OASH_vs_SH.edgeR.DE_results.P0.001_C2.DE.subset", row.names=1)
...
# get gene lengths
gene_lengths = read.table("../../5_Trinotate/gene_lengths.txt", header=T, row.names=1, com='')
gene_lengths = as.matrix(gene_lengths[,1,drop=F])


# get background gene list
background = read.table("salmon.gene.counts.matrix.OASH_vs_SH.edgeR.count_matrix", header=T, row.names=1)
...
GO_info = read.table("../../5_Trinotate/go_annotations.txt", header=F, row.names=1,stringsAsFactors=F)
...
go_enrich_filename = paste("salmon.gene.counts.matrix.OASH_vs_SH.edgeR.DE_results.P0.001_C2.DE.subset", '.GOseq.enriched', sep='')
    ...
    go_depleted_filename = paste("salmon.gene.counts.matrix.OASH_vs_SH.edgeR.DE_results.P0.001_C2.DE.subset", '.GOseq.depleted', sep='')
